    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.approvals.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('permit_approval_name') ? 'is-invalid' : '' }}" type="text" name="permit_approval_name" id="permit_approval_name" value="{{ old('permit_approval_name', '') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('permit_approval_department') ? 'is-invalid' : '' }}" name="permit_approval_department" id="permit_approval_department">
                    ...
                    @foreach(App\Approval::PERMIT_APPROVAL_DEPARTMENT_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('permit_approval_department', '- select -') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
